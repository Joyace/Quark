...
#nav ul a:hover{background:#eee;text-decoration:none}
...
.s .pp a{border-bottom:1px dashed #f00}
...
.ptg a{font-size:.8125rem;margin-right:10px;background:#fff;padding:2px 6px}
...
#f{border-top:1px solid #e1e2e3;padding:3.5rem 0;text-align:center;background:#fff;font-size:.8125rem}
...
<div id="nav" class="w1">
    ...
    <ul><?php $this->widget('Widget_Contents_Page_List')->to($pages); ?><?php while($pages->next()): ?><li><a href="<?php $pages->permalink(); ?>" title="<?php $pages->title(); ?>"><?php $pages->title(); ?></a></li><?php endwhile; ?></ul>
</div>
</div>
<div id="m" class="w1">
